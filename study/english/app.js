...
function renderWords(){ grid.replaceChildren(); words.forEach((item)=>{ const button=document.createElement("button"); button.className="word-card"; button.type="button"; button.setAttribute("aria-pressed",String(known.has(item.id))); if(known.has(item.id)) button.style.boxShadow="inset 0 0 0 3px #ff7f6b"; button.innerHTML=`<span class="emoji">${item.emoji}</span><strong>${item.word}</strong><span>${item.ko}${known.has(item.id)?" · 배웠어요 ✓":""}</span>`; button.addEventListener("click",()=>{known.has(item.id)?known.delete(item.id):known.add(item.id); localStorage.setItem(knownKey,JSON.stringify([...known])); renderWords();}); grid.append(button); }); }
...
let quizIndex=0;
function renderQuiz(){const answer=words[quizIndex%words.length]; document.getElementById("englishQuestion").textContent=`${answer.emoji}  무엇일까요?`; const choices=[answer,...words.filter(x=>x.id!==answer.id).sort(()=>Math.random()-.5).slice(0,2)].sort(()=>Math.random()-.5); const options=document.getElementById("englishOptions"); options.replaceChildren(); document.getElementById("englishFeedback").textContent=""; choices.forEach((item)=>{const button=document.createElement("button"); button.type="button"; button.textContent=item.word; button.addEventListener("click",()=>{if(item.id===answer.id){document.getElementById("englishFeedback").textContent=`Great! ${answer.word}는 ‘${answer.ko}’예요. 🎉`; quizIndex+=1; setTimeout(renderQuiz,900);}else{document.getElementById("englishFeedback").textContent="한 번 더 생각해 볼까요?";}}); options.append(button);});}
